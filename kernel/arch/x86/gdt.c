 ...
 * sets up the Global Descriptor Table with 5 entries:
...
/* loaded into the GDTR register — points to the table */
typedef struct __attribute__((packed)) {
    uint16_t limit;        /* table size in bytes minus 1           */
    uint32_t base;         /* linear address of first entry         */
} gdt_descriptor_t;
...
void gdt_init(void)
{
    ...
    /* 0: null — always required, CPU will fault without it */
